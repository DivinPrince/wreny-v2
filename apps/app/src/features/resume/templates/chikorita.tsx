...
function Link({ url, icon, iconOnRight, label, className }: Readonly<LinkProps>) {
  ...
  return (
    <div className="flex items-center gap-x-1.5">
      {!iconOnRight && (icon ?? <i className="ph ph-bold ph-link text-primary group-[.sidebar]:text-white" />)}
      <a
        href={url.href}
        target="_blank"
...
export function Chikorita({ columns, isFirstPage = false }: Readonly<TemplateProps>) {
