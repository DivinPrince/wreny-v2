...
import type { Award } from '@repo/core/schemas'
import { defaultAward } from '@repo/core/schemas'
...
function empty(): Award {
  return { ...defaultAward, id: generateEditorId() }
}

function toForm(item: Award): FormState {
      ...
      <button type="button" className="min-w-0 flex-1 text-left" onClick={() => onSelect(item.id)}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <p className="font-semibold truncate">{item.title || 'Award'}</p>
            <p className="text-sm text-muted-foreground truncate">{item.awarder}</p>
            {item.date.trim() ? (
...
export function AwardsStep() {
  ...
  useEffect(() => {
    const next = resume.sections.awards.items
    const fb = next[0]?.id ?? ''
    setItems(next)
  ...
  async function persist(nextItems: Award[], nextSel: string) {
    const nextResume = cloneResumeDocument(resume)
    nextResume.sections.awards.items = nextItems
      ...
      <div className="flex flex-col gap-3">
        ...
        <SectionChromeSettings
          ...
          onChange={(v) => {
            void (async () => {
              const nextResume = cloneResumeDocument(resume)
              Object.assign(nextResume.sections.awards, v)
              await saveResume({ resume: nextResume, title })
            })()
          }}
        />
          ...
          {selected ? (
            <div className="flex flex-col gap-5">
              <div className="flex items-start justify-between gap-4">
                <h3 className="text-2xl font-semibold">{form.title || 'New award'}</h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    value={form.awarder}
                    onChange={(e) => setForm((f) => ({ ...f, awarder: e.target.value }))}
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor={`award-start-${selected.id}`}>Start date</Label>
                  <MonthPicker
                    id={`award-start-${selected.id}`}
                ...
                <div className="space-y-2">
                  <Label htmlFor={`award-end-${selected.id}`}>End date</Label>
                  <MonthPicker
                    id={`award-end-${selected.id}`}
                    value={form.endDate}
