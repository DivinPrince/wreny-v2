...
import type { Publication } from '@repo/core/schemas'
import { defaultPublication } from '@repo/core/schemas'
...
function empty(): Publication {
  return { ...defaultPublication, id: generateEditorId() }
}

function toForm(item: Publication): FormState {
      ...
      <button type="button" className="min-w-0 flex-1 text-left" onClick={() => onSelect(item.id)}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <p className="font-semibold truncate">{item.name || 'Publication'}</p>
            <p className="text-sm text-muted-foreground truncate">{item.publisher}</p>
            {item.date.trim() ? (
...
export function PublicationsStep() {
  ...
  useEffect(() => {
    const next = resume.sections.publications.items
    const fb = next[0]?.id ?? ''
    setItems(next)
  ...
  async function persist(nextItems: Publication[], nextSel: string) {
    const nextResume = cloneResumeDocument(resume)
    nextResume.sections.publications.items = nextItems
      ...
      <div className="flex flex-col gap-3">
        ...
        <SectionChromeSettings
          ...
          onChange={(v) => {
            void (async () => {
              const nextResume = cloneResumeDocument(resume)
              Object.assign(nextResume.sections.publications, v)
              await saveResume({ resume: nextResume, title })
            })()
          }}
        />
          ...
          {selected ? (
            <div className="flex flex-col gap-5">
              <div className="flex items-start justify-between gap-4">
                <h3 className="text-2xl font-semibold">{form.name || 'New publication'}</h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor={`pub-start-${selected.id}`}>Start date</Label>
                  <MonthPicker
                    id={`pub-start-${selected.id}`}
                ...
                <div className="space-y-2">
                  <Label htmlFor={`pub-end-${selected.id}`}>End date</Label>
                  <MonthPicker
                    id={`pub-end-${selected.id}`}
                    value={form.endDate}
