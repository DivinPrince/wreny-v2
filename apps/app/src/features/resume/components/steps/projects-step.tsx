...
import type { Project } from '@repo/core/schemas'
import { defaultProject } from '@repo/core/schemas'
...
function empty(): Project {
  return { ...defaultProject, id: generateEditorId() }
}
      ...
      <button type="button" className="min-w-0 flex-1 text-left" onClick={() => onSelect(item.id)}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <p className="font-semibold truncate">{item.name || 'Untitled project'}</p>
            <p className="text-sm text-muted-foreground truncate">{item.description || 'Add description'}</p>
            {item.date.trim() ? (
...
export function ProjectsStep() {
  ...
  useEffect(() => {
    const next = resume.sections.projects.items
    const fb = next[0]?.id ?? ''
    setItems(next)
  ...
  async function persist(nextItems: Project[], nextSel: string) {
    const nextResume = cloneResumeDocument(resume)
    nextResume.sections.projects.items = nextItems
    ...
    <StepPanel className="gap-5">
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground">Highlight products, apps, or work you shipped.</p>
        </div>
        <SectionChromeSettings
          ...
          onChange={(v) => {
            void (async () => {
              const nextResume = cloneResumeDocument(resume)
              Object.assign(nextResume.sections.projects, v)
              await saveResume({ resume: nextResume, title })
            })()
          }}
        />
          ...
          {selected ? (
            <div className="flex h-full flex-col gap-5">
              <div className="flex items-start justify-between gap-4">
                <h3 className="text-2xl font-semibold">{form.name || 'New project'}</h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor={`project-start-${selected.id}`}>Start date</Label>
                  <MonthPicker
                    id={`project-start-${selected.id}`}
                ...
                <div className="space-y-2">
                  <Label htmlFor={`project-end-${selected.id}`}>End date</Label>
                  <MonthPicker
                    id={`project-end-${selected.id}`}
                    value={form.endDate}
                ...
                <div className="flex items-center gap-3 py-1 md:col-span-2">
                  <span className="text-sm font-medium">This project is ongoing</span>
                  <button
                    type="button"
                    role="switch"
