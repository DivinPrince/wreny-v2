...
import type { Language } from '@repo/core/schemas'
import { defaultLanguage } from '@repo/core/schemas'
...
type FormState = { name: string; description: string; level: number }
...
function empty(): Language {
  return { ...defaultLanguage, id: generateEditorId() }
}

function toForm(item: Language): FormState {
  return {
    name: item.name,
      ...
      <button type="button" className="min-w-0 flex-1 text-left" onClick={() => onSelect(item.id)}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <p className="font-semibold truncate">{item.name || 'Language'}</p>
            <p className="text-xs text-muted-foreground">
              {LEVEL_LABELS[item.level] ?? LEVEL_LABELS[1]}
            </p>
          </div>
          <DropdownMenu>
...
export function LanguagesStep() {
  ...
  useEffect(() => {
    const next = resume.sections.languages.items
    const fb = next[0]?.id ?? ''
    setItems(next)
  ...
  async function persist(nextItems: Language[], nextSel: string) {
    const nextResume = cloneResumeDocument(resume)
    nextResume.sections.languages.items = nextItems
    ...
    const next = items.map((item) =>
      item.id === selected.id
        ? {
            ...
            level: Math.min(5, Math.max(0, form.level)),
          }
        : item,
    )
      ...
      <div className="flex flex-col gap-3">
        ...
        <SectionChromeSettings
          ...
          onChange={(v) => {
            void (async () => {
              const nextResume = cloneResumeDocument(resume)
              Object.assign(nextResume.sections.languages, v)
              await saveResume({ resume: nextResume, title })
            })()
          }}
        />
          ...
          {selected ? (
            <div className="flex flex-col gap-5">
              <div className="flex items-start justify-between gap-4">
                <h3 className="text-2xl font-semibold">{form.name || 'New language'}</h3>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <p className="text-sm text-muted-foreground">{LEVEL_LABELS[form.level]}</p>
                </div>
              </div>
            </div>
          ) : (
